...
function Add-LessonClass([string]$Attrs, [string]$ColorClass) {
    $next = [regex]::Replace($Attrs, '\sclass=("|'')[^"'']*\blesson-color-\d+\b[^"'']*\1', '', [Text.RegularExpressions.RegexOptions]::IgnoreCase)
    if ($next -match '\sclass=("|'')') {
        return [regex]::Replace($next, '\sclass=("|'')([^"'']*)\1', { param($match) ' class=' + $match.Groups[1].Value + ($match.Groups[2].Value + ' ' + $ColorClass).Trim() + $match.Groups[1].Value }, [Text.RegularExpressions.RegexOptions]::IgnoreCase)
    }
...
foreach ($folder in $folders) {
    ...
    Get-ChildItem $dir -Filter '*.htm' | Where-Object { $_.BaseName -ne 'Clases' -and $_.BaseName -ne 'Profesores' } | ForEach-Object {
        ...
        $content = [regex]::Replace($content, '<TD([^>]*\bcolspan=\d+(?![^>]*\browspan=)[^>]*)>', {
            param($match)
            return '<TD' + ([regex]::Replace($match.Groups[1].Value, '\sclass=("|'')[^"'']*\blesson-color-\d+\b[^"'']*\1', '', [Text.RegularExpressions.RegexOptions]::IgnoreCase)) + '>'
        ...
        $content = [regex]::Replace($content, '<TD([^>]*\bcolspan=\d+[^>]*\browspan=\d+[^>]*)>(\s*<TABLE>[\s\S]*?</TABLE>)</TD>', {
            ...
            if (-not $key) {
                return [regex]::Replace($match.Value, '\sclass=("|'')[^"'']*\blesson-color-\d+\b[^"'']*\1', '', [Text.RegularExpressions.RegexOptions]::IgnoreCase)
            }
